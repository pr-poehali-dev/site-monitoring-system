...
export const apiClient = {
  async getDocuments(params?: { search?: string; section?: string; limit?: number; offset?: number }) {
  ...
  async runParser(sections: string[], years: number[]) {
    ...
    if (!response.ok) throw new Error('Failed to run parser');
    return response.json();
  },
